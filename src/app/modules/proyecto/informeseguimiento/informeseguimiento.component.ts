...
export class InformeseguimientoComponent implements OnInit {
  ...
  selectionProyecto(proyectoSele: MatSelectionListChange){
    ...
    this.proyectoSelect.actividadeslistProyectos?.forEach(value=>{
      this.onAddRowInfo(value)
      ...
      this.carreraService.getCarrerabyCodigo(String(this.proyectoSelect.codigocarrera)).subscribe(dataC=>{
        ...
        this.anexo1Service.getAnexo1byCedula(this.cedula).subscribe(dataAnexo1=>{
          this.anexo1select=dataAnexo1[0]
            this.anexo3Service.getAnexo3byProyecto(this.proyectoSelect.id).subscribe(dataAn3=>{
              this.anexo3select=dataAn3.filter(value => value.estado=='AN');
              this.anexo3select.forEach(value => {this.onAddRowEst(value)});
              this.anexo1Service.getAnexo1byIdProyecto(this.proyectoSelect.id).subscribe(dataAn1=>{
                this.anexo1selectLista=dataAn1
                this.anexo1selectLista.forEach(value => {this.onAddRow(value)})
...
onAddRowOb(objetivosEspecificosInforme: ObjetivosEspeciicoslistProyecto ){
  ...
  createItemFormGroup(docentesParticipantes:Anexo1): FormGroup {
    return this._formBuilder.group({
      nombres:[docentesParticipantes.nombreDelegado,Validators.required],
      cedula:[docentesParticipantes.cedulaDelegado,Validators.required],
      carrera:[docentesParticipantes.nombreCarrera,Validators.required],
      numeroHoras:['',Validators.required]
    });
  }
  createItemFormGroupEst(estudiantesParticipantes: Anexo3): FormGroup {
    return this._formBuilder.group({
      nombres:[estudiantesParticipantes.nombresestudiante+""+estudiantesParticipantes.apellidosestudiante,Validators.required],
      cedula:[estudiantesParticipantes.cedula,Validators.required],
      //codigo:[estudiantesParticipantes.cedula,Validators.required], aumentar
      carrera:[estudiantesParticipantes.nombrecarrera,Validators.required],
      numeroHoras:['',Validators.required]
    });
  }

  createItemFormGroupInfo(actividadesInformeSeguimientoRequest: actividadeslistProyectos): FormGroup {
    return this._formBuilder.group({
      actividades:[actividadesInformeSeguimientoRequest.descripcion, Validators.required],
      porcentajeCumplimiento:['', Validators.required],
      fechaEjecucion:['', Validators.required],
      responsableEjecucion:['', Validators.required],
